...
## Daily challenge
One attempt per day, exactly `SprintRules.DAILY_QUESTIONS` (8) questions, Medium rules. Question *k* uses generator level `SprintRules.dailyLevel(k)` = 1 + 3k (stages 1,1,2,2,3,4,4,5) and is seeded by `epochDay` + k (`DailyChallenge.random`), so all players get the same sequence. **One attempt per question**: a wrong answer (or burst timeout) reveals the solution ~1.8 s then moves on, no time penalty, no skips. **No countdown**: the run lasts 8 questions; the elapsed time (`elapsedMs`, saved as `durationMs`) is only a tie-breaker. Score/XP still use the normal reward formula. Stored as `DAILY_SPRINT` with `answerLog` = one char per question (G/R); `level` = number of correct answers. Shared as `🚇 6/8 · 2:14` plus one square per question (🟩/🟥). Known limits: reinstalling lets a player replay it; editing the stations data file changes the day's puzzles (see `TODO.md`).
