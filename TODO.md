...
- [x] **P2 — Boucle de rétention** : streak + badges existent en base mais peu visibles ; ajouter un daily challenge partageable (résultat texte/emoji façon Wordle) — meilleur levier organique pour un jeu de niche.
  - ✅ 2026-09-21 : **Daily Challenge** (mode `DAILY_SPRINT`, bouton sur l'accueil, une tentative par jour, règles Medium, défis identiques pour tous : graine = jour + niveau + skips utilisés), streak et niveau visibles sur l'accueil, **partage** texte façon Wordle (🟩🟥⬜) depuis les résultats et l'accueil, bonus de 50 XP. Reste : notifications de rappel, classement.
...
- [ ] **Données Etalab** : la Licence Ouverte v2.0 (Etalab) impose de citer la source **et la date de dernière mise à jour** ; la mention in-app (Settings > Legal) n'a pas encore de date. Confirmer que `strasbourg_stations.json` provient bien du jeu « Stations de tram » (data.strasbourg.eu, modifié le 2026-06-17 selon le portail) et le dire dans la mention ; sinon sourcer ou remplacer les noms/ordres.
