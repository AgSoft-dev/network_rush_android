...
- [ ] **Signature release** : `signingConfig` branché (propriétés `nrStoreFile/nrStorePassword/nrKeyAlias/nrKeyPassword` dans `~/.gradle/gradle.properties`, règles R8 vérifiées). Reste : créer le keystore d'upload (`keytool`, hors dépôt, 2 sauvegardes), fixer `versionName`, `bundleRelease`, tester l'APK R8 signé sur appareil, activer Play App Signing à l'upload.
